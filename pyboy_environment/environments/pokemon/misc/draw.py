...
def abstract_frame_with_templates(frame):
    ...
    templates = {
        "player_back": (
            cv2.imread(os.path.join(script_dir, "player_back_dark.png"), 0),
            (0, 0, 255),
        ),  # Red color for player_back
        "player_left": (
            cv2.imread(os.path.join(script_dir, "player_left_dark.png"), 0),
            (0, 0, 255),
        ),  # Green color for player_left
        "player_right": (
            cv2.imread(os.path.join(script_dir, "player_right_dark.png"), 0),
            (0, 0, 255),
        ),  # Blue color for player_right
        "player_front": (
            cv2.imread(os.path.join(script_dir, "player_front_dark.png"), 0),
            (0, 0, 255),
        ),
        "player_back_white": (
            cv2.imread(os.path.join(script_dir, "player_back_white.png"), 0),
            (0, 0, 255),
        ),  # Red color for player_back
        "player_left_white": (
            cv2.imread(os.path.join(script_dir, "player_left_white.png"), 0),
            (0, 0, 255),
        ),  # Green color for player_left
        "player_right_white": (
            cv2.imread(os.path.join(script_dir, "player_right_white.png"), 0),
            (0, 0, 255),
        ),  # Blue color for player_right
        "player_front_white": (
            cv2.imread(os.path.join(script_dir, "player_front_white.png"), 0),
            (0, 0, 255),
        ),  # Cyan color for player_front
        "doc": (
            cv2.imread(os.path.join(script_dir, "doc.png"), 0),
            (114, 255, 0),
        ),  # Cyan color for player_front
        "grass": (
            cv2.imread(os.path.join(script_dir, "grass.png"), 0),
            (0, 255, 255),
        ),  # Yellow color for grass
        ...
        "fence": (
            cv2.imread(os.path.join(script_dir, "fence.png"), 0),
            (128, 128, 128),
        ),  # Gray color for fence
        "door1": (
            cv2.imread(os.path.join(script_dir, "door1.png"), 0),
            (0, 128, 128),
        ),  # Teal color for door1
        "door2": (
            cv2.imread(os.path.join(script_dir, "door2.png"), 0),
            (128, 0, 128),
        ),  # Purple color for door2
...
def calculate_relative_positions(player_position, object_positions):
    ...
    for category, positions in object_positions.items():
        ...
        relative_positions[category] = [
            np.subtract(pos, player_position) for pos in positions
        ]
...
def count_objects(object_positions):
    object_counts = {
        category: len(positions) for category, positions in object_positions.items()
    }
...
def frame_to_states(frame):
            ...
            if "player" in name:
                ...
                object_positions[
                    (
                        "enemies"
                        if "enemy" in name
                        else "items" if "item" in name else "obstacles"
                    )
                ].append(center)
    ...
    environmental_features = [
        0
    ]  # This should be replaced with actual environmental data
        ...
        if key == "relative_positions":
            ...
            for category, positions in value.items():
                formatted_state[key][category] = {
                    ...
                    "sample": [
                        list(pos) for pos in positions[:sample_size]
                    ],  # Convert first few positions to lists
                }
